...
<%
    ...
    String dealerName = "", dealerPhone = "", dealerAddress = "", dealerMarathiName = "";
    ...
    try (Connection conn = DBConnection.getConnection()) {
        ...
        if (rs.next()) {
            ...
            dealerMarathiName = rs.getString("marathi_name");
            if (dealerMarathiName == null) dealerMarathiName = "";
    ...
    String displayNameJs   = jsEscape(dealerName);
    String displayMrNameJs = jsEscape(dealerMarathiName.isEmpty() ? dealerName : dealerMarathiName);
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="content-wrapper">
    ...
    <div class="no-print">
        <a href="view_dealers.jsp" class="back-link" data-i18n="btn.back_dealers">← Back to Dealer List</a>
    </div>

    <!-- Dealer Info Card -->
    <div class="detail-info-card no-print" style="border-left-color: #f5a623;">
        <div class="info-item">
            <span class="info-label" data-i18n="vd.th_id">ID</span>
            <span class="info-value">#<%= dealerId %></span>
        </div>
        <div class="info-item">
            <span class="info-label" data-i18n="vd.th_name">Dealer Name</span>
            <span class="info-value">
                🏬
                <span class="lang-name-en"><%= dealerName %></span>
                <span class="lang-name-mr" style="display:none;"><%= dealerMarathiName.isEmpty() ? dealerName : dealerMarathiName %></span>
            </span>
        </div>
        <div class="info-item">
            <span class="info-label" data-i18n="vd.th_phone">Phone</span>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>
                        <span class="lang-name-en">Stock Added</span>
                        <span class="lang-name-mr" style="display:none;">स्टॉक जोडला</span>
                    </th>
            ...
            %>
                <tr>
                    ...
                    <td>
                        <% if (isAdd && qty > 0) { %>
                        <span class="qty-pill">↑ <%= qty %> <span class="lang-name-en">unit<%= qty != 1 ? "s" : "" %></span><span class="lang-name-mr" style="display:none;">नग</span></span>
            ...
            <%
                    ...
                    if (!hasTxn) {
            %>
                <tr><td colspan="8" class="no-data" data-i18n="det.no_txn">No transactions found for this dealer.</td></tr>
            <%  }
                } catch (Exception e) { %>
...
var DEALER_NAME_EN = "<%= displayNameJs %>";
var DEALER_NAME_MR = "<%= displayMrNameJs %>";
...
function buildPopupHtml(txns, periodLabel, dateStr) {
    ...
    var dlrName    = isMr ? DEALER_NAME_MR : DEALER_NAME_EN;

    var labels = isMr ? {
        title: 'डीलर व्यवहार विवरण',
        dlrLabel: 'डीलरचे नाव', dlrId: 'डीलर क्र.', phone: 'दूरध्वनी क्र.',
        addr: 'पत्ता', credit: 'सध्याची उधार शिल्लक',
        added: 'एकूण जोडलेली', settled: 'एकूण परतफेड', txns: 'व्यवहार',
        net: 'या कालावधीचे एकूण',
        th: ['#','दिनांक','प्रकार','उत्पाद / वस्तू','स्टॉक जोडला','देयक पद्धत','रक्कम (₹)'],
        add: '+ जोडली', settle: '✓ परतफेड',
        cash: 'रोख', online: 'ऑनलाइन', footer: 'डीलर व्यवहार विवरण'
    } : {
        title: 'Dealer Transaction Statement',
        dlrLabel: 'Dealer Name', dlrId: 'Dealer ID', phone: 'Phone No.',
        addr: 'Address', credit: 'Current Credit Balance',
        added: 'Total Added', settled: 'Total Settled', txns: 'Transactions',
        net: 'Net for Period',
        th: ['#','Date','Type','Product / Item','Stock Added','Payment Mode','Amount (Rs.)'],
        add: '+ ADD', settle: '✓ SETTLE',
        cash: 'Cash', online: 'Online', footer: 'Dealer Transaction Statement'
    };

    var rows = ''; var sumAdded = 0, sumSettled = 0, sNo = 1;
    ...
    return '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
        '<title>' + labels.title + ' - ' + dlrName + '</title>' +
        ...
        '<div style="font-size:14px;font-weight:600;color:#4a5568;margin-top:4px;">' + labels.title + '</div>' +
        '<div style="font-size:12px;color:#94a3b8;margin-top:5px;">' + periodLabel + '</div>' +
        '</div>' +
        '<div class="info-box">' +
        '<div class="info-item"><span class="info-label">' + labels.dlrLabel + '</span><span class="info-value">' + dlrName + '</span></div>' +
        '<div class="info-item"><span class="info-label">' + labels.dlrId + '</span><span class="info-value">#' + DEALER_ID + '</span></div>' +
        '<div class="info-item"><span class="info-label">' + labels.phone + '</span><span class="info-value">' + DEALER_PHONE + '</span></div>' +
        '<div class="info-item"><span class="info-label">' + labels.addr + '</span><span class="info-value">' + DEALER_ADDR + '</span></div>' +
        '<div class="info-item"><span class="info-label">' + labels.credit + '</span><span class="info-value credit">&#8377; ' + DEALER_CREDIT + '</span></div>' +
        '</div>' +
        '<div class="summary">' +
        ...
        '<tfoot><tr><td colspan="6" style="text-align:right;">' + labels.net + '</td>' +
        '<td style="text-align:right;">&#8377; ' + net.toFixed(2) + '</td></tr></tfoot></table>' +
...
function printAll() {
    var lang = getLang(); var isMr = lang === 'mr';
    if (allTxns.length === 0) { alert(isMr ? 'कोणताही व्यवहार उपलब्ध नाही.' : 'No transactions found for this dealer.'); return; }
    var now = new Date();
    var dateStr = now.toLocaleDateString('en-IN', {weekday:'long',year:'numeric',month:'long',day:'numeric'});
